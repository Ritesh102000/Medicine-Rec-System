...
def predict():
    if request.method=='POST':
        ...
        encoded_symptom= [s.strip() for s in  symptom.split(',')]
        encoded_symptom = [symptom.strip("[]' ") for symptom in encoded_symptom]
        prediction = get_predicted_value(encoded_symptom)
        
        desc,pre,med,die,wrkout = helper(prediction)
        
        return render_template('index.html',prediction=prediction,discription =desc,precaution=pre,medicine=med,diet=die,workout=wrkout)
